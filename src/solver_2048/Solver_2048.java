...
public class Solver_2048 
{
    ...
    static private final int UP=8,DOWN=2,LEFT=4,RIGHT=6;
    ...
    static private class Node
    {
       ...
       private void computeGradientScore()
       {
           ...
           score1=getHorizontalGradientScore(0, 1, n , 0);
           score2=getHorizontalGradientScore(0, 1, n, 1);
           score3=getHorizontalGradientScore(n-1, -1, n, 0);
           score4=getHorizontalGradientScore(n-1, -1, n, 1);
           
           score5=getVerticalGradientScore(0, 1, n, 0);
           score6=getVerticalGradientScore(0, 1, n, 1);
           score7=getVerticalGradientScore(n-1, -1, n, 0);
           score8=getVerticalGradientScore(n-1, -1, n, 1);
       ...
       private void computeClusterScore()
       {
           ...
           for(i=0;i<n;i++)
           {
               for(j=0;j<n;j++)
               {
                   ...
                   if(count!=0)
                    clusterScore+=((thisone));//try without averaging too
    ...
    public static void main(String[] args) throws IOException {
        ...
        int dimension=4;
        PrintStream writer = new PrintStream(file),writer2=new PrintStream(System.out);
        ...
        while(game.isGameOver()==false )//&& game.getLargestTileValue()!=2048)
        {
            ...
            if(who==0)
            {
                ...
                nextBestMove=currentGame.getNextBestMove(2);
                
            }
            else
            {
                System.out.println("Next Move? (Up:8 Left:4 Right:6 Down:2 AIMove:0 LetAIPlayAll:-1) ");
        ...
        int randomTileValue=game.getLastRandomTileValue();
            ...
            if(nextBestMove==DOWN)
            {
               ...
               writer2.println("Last Move: DOWN");
            }
            else if(nextBestMove==UP)
            {
               writer.println("Last Move: UP");
               writer2.println("Last Move: UP");
            }
            else if(nextBestMove==LEFT)
            {
               writer.println("Last Move: LEFT");
               writer2.println("Last Move: LEFT");
            }
            else if(nextBestMove==RIGHT)
            {
               writer.println("Last Move: RIGHT");
               writer2.println("Last Move: RIGHT");
            }
          ...
          //  System.out.println("Move Count: "+(game.getCountMoves())+" \nMaximum Tile :"+game.getLargestTileValue()); 
              ...
              writer.println("Random tile "+randomTileValue+" at  "+randomTilePlace[0]+","+randomTilePlace[1]);
                ...
                writer2.println("Move Count: "+game.getCountMoves()+" \nMaximum Tile :"+game.getLargestTileValue()+"\nScore: "+game.getScore()); 
                 writer2.println("Random tile "+randomTileValue+" at  "+randomTilePlace[0]+","+randomTilePlace[1]);
                
              if(who==0)
              {
                 writer.println("Time since beginning :"+(double)(currentTime-starttime)/1000+" seconds \nTime for this move: "+(double)(currentTime-lastTime)/1000+"seconds");
                 writer2.println("Time since beginning :"+(double)(currentTime-starttime)/1000+" seconds \nTime for this move: "+(double)(currentTime-lastTime)/1000+"seconds");
              }
